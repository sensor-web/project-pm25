...
Regions.prototype.setCapitals = function (capitals) {
	_capitals = capitals;
};
...
Regions.prototype.listByNearestCoords = function (ctx, coords, countryCode, id) {
	var db = this._db;
	coords.latitude = Number.parseFloat(coords.latitude);
	coords.longitude = Number.parseFloat(coords.longitude);
	return new Promise(function(resolve, reject) {
		r.table(TABLE_NAME)
		.getNearest(r.point(coords.longitude, coords.latitude), {index: 'coords', unit: 'km', maxDist: 500})
		.filter(r.row('doc')('country_code').eq(countryCode)
			.and(r.row('doc')('average')('create_time').gt(getHourAgo(1.5)))
...
Regions.prototype.listNearest = function (ctx) {
    ...
    return new Promise(function (resolve, reject) {
	    ...
	    if (null != location) {
		    ...
		    if (undefined == location.city && _capitals[location.country_code]) {
		    	instance.getBySlug(ctx, _capitals[location.country_code]).then(function (region) {
					instance.listByNearestCoords(ctx, {latitude: region.coords.latitude, longitude: region.coords.longitude}, location.country_code)
				...
				.then(function (regions) {
					if (undefined != regions && regions.length > 0) {
					    ...
					    resolve([{
					        ...
					        region_name: (undefined == location.city) ? location.country_name:location.city,
					        country_name: location.country_name,
					        coords: {
...
Regions.prototype.getCoords = function (ctx) {
	var instance = this;
    return new Promise(function (resolve, reject) {
	    var location = _geoip.lookupSync(ctx.ip);
	    if (null != location) {
	    	var result = {coords: {latitude: location.latitude, longitude: location.longitude}};
	    	if (location.city) {
