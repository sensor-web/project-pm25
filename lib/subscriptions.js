...
Subscriptions.prototype.updateSids = function (ctx) {
	var db = this._db;
	return new Promise(function(resolve, reject) {
		r.table(TABLE_NAME)
		.run(db.connection).then(function(cursor) {
			cursor.toArray().then(function (subscriptions) {
				var updateQ = [];
				for (var subscription of subscriptions) {
					updateQ.push(r.table(TABLE_NAME).get(subscription.id).update({sid: md5(subscription.email)}));
				}
				batchQuery(ctx, db, updateQ).then(resolve).catch(reject);
			})
		}).catch(reject);
	});
};
...
var convert2Mailchimp = function (ctx, subscription, newSub) {
	...
	if (undefined != subscription.coords) {
		sub.merge_fields.MYGPS = subscription.coords['coordinates'][1] + ',' + subscription.coords['coordinates'][0];
	}
	if (undefined != subscription.latitude && undefined != subscription.longitude) {
		sub.merge_fields.MYGPS = subscription.latitude + ',' + subscription.longitude;
	}
...
var convert2Json = function (ctx, subscription) {
	if (subscription.coords != undefined) {
		var coords = {
			longitude: subscription.coords['coordinates'][0],
			latitude: subscription.coords['coordinates'][1]
		};
