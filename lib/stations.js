...
Stations.prototype.updateData = function (ctx, id, data) {
	var db = this._db;
...
var convert2Json = function (ctx, station) {
	...
	if (undefined != station.data.pm2_5) {
		...
		station.data.status = aqi.getAQIStatus(ctx, station.data.pm2_5, station.address.country_code);
		station.data.statusText = aqi.getAQIStatusText(ctx, station.data.pm2_5, station.address.country_code);
	}
