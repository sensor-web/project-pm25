...
Stations.prototype.countByRegionType = function (regionType) {
	var db = this._db;
	return r.table(TABLE_NAME).filter(r.row('data')('create_time').gt(getHourAgo())).group(r.row('address')('country_code'), r.row('address')('country'), r.row('address')(regionType)).count().run(db.connection);
};

Stations.prototype.averageByRegionType = function (regionType, dataType) {
	var db = this._db;
	return r.table(TABLE_NAME).filter(r.row('data')('create_time').gt(getHourAgo()).and(r.row('data')(dataType))).group(r.row('address')('country_code'), r.row('address')('country'), r.row('address')(regionType)).getField('data')(dataType).avg().run(db.connection);
};
...
Stations.prototype.getBySlug = function (slug) {
	...
	return new Promise(function(resolve, reject) {
		...
		.run(db.connection).then(function(cursor) {
			cursor.next().then(function(station) {
				resolve(convert2Json(station));
			});
		}).catch(reject);
	});
};
...
Stations.prototype.listByNearestCoords = function (coords, id) {
	...
	return new Promise(function(resolve, reject) {
		...
		.filter(r.row('doc')('data')('create_time').gt(getHourAgo())
...
Stations.prototype.listByRegionTop = function (regionType, regionName, dataType) {
	...
	return new Promise(function(resolve, reject) {
		r.table(TABLE_NAME).filter(
			r.row('data')('create_time').gt(getHourAgo())
 ...
 * private functions 
 */
var getHalfHourAgo = function () {
 	var hourAgo = new Date();
 	hourAgo.setTime(hourAgo.getTime() - 1000 * 60 * 30)
	return hourAgo;
};

var getHourAgo = function () {
 	var hourAgo = new Date();
 	hourAgo.setTime(hourAgo.getTime() - 1000 * 60 * 90)
	return hourAgo;
};
...
var convert2Db = function (station) {
	...
	if (station.coords != undefined && station.coords.latitude != undefined && station.coords.latitude != undefined) {
		station.coords = r.point(station.coords.longitude, station.coords.latitude);
	}
