...
Stations.prototype.listByNearestCoords = function (ctx, coords, id) {
	...
	return new Promise(function(resolve, reject) {
		r.table(TABLE_NAME)
		.getNearest(r.point(coords.longitude, coords.latitude), {index: 'coords', maxDist: 3000, unit: 'm'})
...
Stations.prototype.updateData = function (ctx, id, data, legacy) {
	...
	return new Promise(function(resolve, reject) {
		if (legacy) {
			r.table(TABLE_NAME).filter({'device_key': 'legacy_'+id, 'party': 'legacy'}).count().do(function(stationExists) {
				return r.branch(r.expr(stationExists).gt(0), 
					r.table(TABLE_NAME).filter({'device_key': 'legacy_'+id}).update({data: data}),
					{replaced: 0});
			}).run(db.connection).then(function(result) {
...
var convert2Db = function (ctx, station) {
	...
	if (station.data != undefined) { 
		if (station.data.pm1 != undefined) {
			station.data.pm1 = Number.parseFloat(station.data.pm1);
		}
		if (station.data.pm2_5 != undefined) {
			station.data.pm2_5 = Number.parseFloat(station.data.pm2_5);
		}
		if (station.data.pm10 != undefined) {
			station.data.pm10 = Number.parseFloat(station.data.pm10);
		}
		if (station.data.create_time != undefined) {
